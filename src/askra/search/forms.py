...
class ProfileSearchBasicForm(forms.Form):
    ...
    branches_list = [(branch.name, branch.name) for branch in Branch.objects.all() if branch.name]
    ...
    year_of_passing = forms.ChoiceField(choices=[("", "All")] + [(x, x) for x in range(1964, 2009)]
                                        + [("2011", "2011"), ("2013", "2013"), ("2014", "2014"),
                                           ("2015", "2015"), ("2016", "2016")], required=False)
...
class CustomSearchForm(SearchForm):
    ...
    q = forms.CharField(required=False, label='Search',
                        widget=forms.TextInput(attrs={'placeholder':
                                                      'Search across thousands of profiles'}))
