...
class ShowProfileView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['education_details'] = HigherEducationDetail.objects.filter(
...
class EditProfileView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        studentformset = self.StudentFormSet(
            queryset=StudentSection.objects.filter(
                userprofile=user_profile),
            prefix='student')
        employmentformset = self.EmploymentFormSet(
            queryset=EmploymentDetail.objects.filter(
                userprofile=user_profile),
            prefix='employment')
        highereducationformset = self.HigherEducationFormSet(
            queryset=HigherEducationDetail.objects.filter(userprofile=user_profile),
            prefix='highereducation')
        facultyformset = self.FacultyFormSet(
            queryset=FacultySection.objects.filter(
                userprofile=user_profile),
            prefix='faculty')
...
#            formset.forms.append(form)
#
# return render(request, "edit_profile_employment.html",
# {'formset':formset, 'profile_id':profile_id, })
...
# return sorted([x for x in facets if x[1]], key=lambda x: x[0]) +
