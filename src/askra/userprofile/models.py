...
class UserProfile(models.Model):
    ...
    role = models.IntegerField(
        "User Role",
        choices=USER_ROLES,
        help_text="Choose the appropriate userRole")
    ...
    #Should contain the city id, state id or country id
    ...
    def set_branch(self, branch, course, specialisation):
        if course != '' and course is not None:
            ...
            try:
                ...
                courseDataSynonyms = Synonym.objects.filter(
                    value__iexact=course,
                    resourcetype=RT_COURSE,
                    aliastype=SYNONYM)
        ...
        if specialisation != '' and specialisation is not None:
        ...
        if branch != '' and branch is not None:
            ...
            if not branchData:
                branchSynonyms = Synonym.objects.filter(
                    value__iexact=branch,
                    resourcetype=RT_BRANCH,
                    aliastype=SYNONYM)
                if branchSynonyms:
...
class EmploymentDetail(models.Model):
    ...
    def get_employment_period(self):
        if((self.date_of_joining is None and self.date_of_leaving is None) or (self.date_of_joining == "" and self.date_of_leaving == "")):
            ...
            return (
                str(self.date_of_joining.strftime("%b %Y")) + " - " + str(self.date_of_joining.year) +
                " - " + str(
                    self.date_of_leaving.strftime(
                        "%b")) + ", " + str(
                            self.date_of_leaving.year)
            )
...
class XlsUpload(models.Model):
    ...
    def save(self, **kwargs):
        ...
        for s in wb.sheets():
            log.debug(
    "Sheet: [" + s.name + "], Num Rows: [" + str(
        s.nrows) + "], Num Cols: [" + str(
            s.ncols) + "]")
    ...
    "The email ID [" +
     email +
     "] is invalid. Putting it in junk.")
                            junk['email'] = email
                            #For invalid email ID treat it as if the email ID is not present
                            email = ''
                    else:
                    ...
                    if(colIndex.get('mobile') is not None):
                        ...
                        # to remove a trailing .0 for float numbers
                        if isinstance(mobilestr, float):
                    ...
                    if(colIndex.get('phone') is not None):
                        ...
                        if isinstance(phonestr, float):  # to remove a trailing .0 for float numbers
                    ...
                    if(colIndex.get('office phone') is not None):
                        ...
                        #to remove a trailing .0 for float numbers
                        if isinstance(offphonestr, float):
    ...
    "The roll no [" +
     ...
     "] is invalid. Ignoring it.")
                        ...
                        elif isinstance(rollnostr, float):
                            rollno = int(rollnostr)
                        else:
                            rollno = rollnostr
                    ...
                    if(colIndex.get('specialisation') is not None):
                        specialisation = unicode(
                            s.cell(row, colIndex['specialisation']).value).strip()

                    # Currently email and name are being used as keys to check duplicates, so
                    # do not accept profile with blank email ID and name
                    if(email == '' and name == ''):
...
#                    log.debug("Mobile: ["+mobile+"], City: ["+city+"], Address: ["+address+"], YOG: ["+ unicode(yog) +"], RollNo: ["+ unicode(rollno) + "]")
                    ...
                    try:
                        if(email):  # Email is first key, name is second key when email is empty
                            ...
                            text = "A profile with email ID [" + email + \
                                "] already exists. Updated the other fields."
                        elif(name):
                            user_profile = UserProfile.objects.get(
    first_name=first_name,
     last_name=last_name,
     studentsection__year_of_graduation=yog)
                            text = "A profile with name [" + name + \
                                "] already exists. Updated the other fields."
                        ...
                        user_profile = UserProfile(first_name=first_name,
                                                   ...
                                                   role=ALUMNI,
                                                   date_of_birth=dob)
                        text = "Added new profile with email ID [" + \
                            email + "] and name [" + name + "]"

                    except MultipleObjectsReturned:
                        text = "Seems like multiple profiles exist for [" + \
                            name + "], email [" + email + "]. Ignoring this row."
    ...
    userprofile=user_profile,
     ...
     value=value)
                    ...
                    try:
                        ...
                        log.debug(
                            "Student Section already exists. Updating values for [" + name + "]")
                    except ObjectDoesNotExist:
                        student_section = StudentSection(userprofile=user_profile,
                                                         roll_num=rollno,
                                                         year_of_graduation=yog)
                        log.debug(
    "Creating new student section for [" +
