...
class YOGListFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        ...
        for year_of_graduation in StudentSection.objects.values('year_of_graduation').distinct():
            retval += (
                (str(year_of_graduation["year_of_graduation"]),
                 str(year_of_graduation["year_of_graduation"])),
            )
        ...
        if self.value():
            try:
                if ',' in self.value():
                    ...
                    selectedBranch = Branch.objects.get(
                        course__iexact=course.strip(),
                        branch__iexact=branch.strip())
                else:
                    selectedBranch = Branch.objects.get(
                        Q(course__iexact=self.value()) | Q(branch__iexact=self.value()))
...
class UserProfileAdmin(admin.ModelAdmin):
    fieldsets = [(
        "Basic Details", {"fields": (('user', 'role', 'profile_status'), ('first_name', 'last_name', 'gender', 'slug'),
                                     ('email', 'phone_number', 'date_of_birth', ), ('photo', 'address',), ('about',),)}),
                 ("Website Urls",
                  {"fields": (('linked_url', 'facebook_url',), ('website_url', 'twitter_url'))}),
                 ("Tags", {"fields": ('tags',)})]
    raw_id_fields = ('user',)
    list_display = (
        'id',
    ...
    inlines = (
        StudentSectionInline, EmploymentDetailInline, HigherEducationDetailInline, FacultySectionInline,
